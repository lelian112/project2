...
const Setting = () => {
  ...
  const checkpassword = async (e) => {
    ...
    try {
      const response = await axios.get(`/user/editinfo/${localStorage.id}`);
    ...
    <>
      <div id='settingContainer'>
        <div className='settingTitle'>
          <h3>회원 정보 수정</h3>
          <p>회원님의 소중한 정보를 정보를 안전하게 관리하세요.</p>
        </div>
        ...
        <div className='settingContent'>
          ...
          <p>
            회원님의 개인정보 보호를 위한 본인 확인 절차이오니, 감자밭 로그인 시
            사용하시는 비밀번호를 입력해주세요.
          </p>
          <input
            type='text'
          ...
          <button id='settingButton' onClick={checkpassword}>
            Next
          </button>
        </div>
      </div>
